...
struct BlogAdminController {
    ...
    func createUserHandler(_ request: Request) throws -> ResponseRepresentable {
        return try viewFactory.createUserView(editing: false, errors: nil, name: nil, username: nil, passwordError: nil, confirmPasswordError: nil, resetPasswordRequired: nil, userId: nil, profilePicture: nil, twitterHandle: nil, links: nil, biography: nil, tagline: nil, loggedInUser: request.user())
    }
    ...
    func createUserPostHandler(_ request: Request) throws -> ResponseRepresentable {
        ...
        let links = request.data["inputLinks"]?.string
        ...
        if !(createUserRawErrors?.isEmpty ?? true) {
            return try viewFactory.createUserView(editing: false, errors: createUserRawErrors, name: rawName, username: rawUsername, passwordError: passwordRawError, confirmPasswordError: confirmPasswordRawError, resetPasswordRequired: resetPasswordRequired, userId: nil, profilePicture: profilePicture, twitterHandle: twitterHandle, links: links, biography: biography, tagline: tagline, loggedInUser: request.user())
        }
        ...
        if !(createUserErrors?.isEmpty ?? true) {
            return try viewFactory.createUserView(editing: false, errors: createUserErrors, name: name, username: username, passwordError: passwordError, confirmPasswordError: confirmPasswordError, resetPasswordRequired: resetPasswordRequired, userId: nil, profilePicture: profilePicture, twitterHandle: twitterHandle, links: links, biography: biography, tagline: tagline, loggedInUser: request.user())
        }

        // We now have valid data
        let hashedPassword = try BlogUser.passwordHasher.make(password)
        let newUser = BlogUser(name: name, username: username.lowercased(), password: hashedPassword, profilePicture: profilePicture, twitterHandle: twitterHandle, links: links, biography: biography, tagline: tagline)
    ...
    func editUserPostHandler(request: Request) throws -> ResponseRepresentable {
        ...
        if !(saveUserRawErrors?.isEmpty ?? true) {
            return try viewFactory.createUserView(editing: true, errors: saveUserRawErrors, name: rawName, username: rawUsername, passwordError: passwordRawError, confirmPasswordError: confirmPasswordRawError, resetPasswordRequired: resetPasswordRequired, userId: user.id, profilePicture: profilePicture, twitterHandle: twitterHandle, links: links, biography: biography, tagline: tagline, loggedInUser: request.user())
        }
        ...
        if !(saveUserErrors?.isEmpty ?? true) {
            return try viewFactory.createUserView(editing: true, errors: saveUserErrors, name: name, username: username, passwordError: passwordError, confirmPasswordError: confirmPasswordError, resetPasswordRequired: resetPasswordRequired, userId: user.id, profilePicture: profilePicture, twitterHandle: twitterHandle, links: links, biography: biography, tagline: tagline, loggedInUser: request.user())
        }
